...
% Things to store.
lambdas      = zeros([rows(t) max_samples]);
...
gphp_prior_params.mean      = log([15 1]');
...
state = poisson_step( train, 0, T, mcmc_params, ...
                      ...
                      'lambda_max_init', 3, ... %rows(train)/T, ...
                      'gp_jitter', 1e-5);
...
for i=1:max_samples
  ...
  num_thinned(i)  = rows(state.rej_func);
  ...
  if mod(i, 20) == 0
    ...
    % Calculate the MSE
    mse = trapz(t, (mean_lambda-func3(t)).^2);
    ...
    for j=1:10
      ...
      log_lambdas = log(interp1(t, mean_lambda, test{j}));
      
      logprobs(j) = sum(log_lambdas) - volume;
      
    end
    
    fprintf('%d] MSE: %0.4f  LP: %f\n', i, mse, mean(logprobs));
...
mean_lambda = mean(lambdas(:, start:last),2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
useful = lambdas(:,1000:12348);
probs  = zeros([cols(useful) 1]);
for j=1:cols(useful)
